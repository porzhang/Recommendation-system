...
```
- DATA PRE-PROCESSING
  -> convert the genres of the movie_data frame into a useful format for the users, by creating a one-hot encoding to create a matrix that comprises of corresponding genres fro each of the films
```{r}
...
str(genre_mat2)
```
- create a 'searchMatrix' to perform an easy search of the films by specifying the genre present in the list
```{r}
...
-this chunk of code used to overview some of the important parameters that provide us various options for building recommendation systems for movies 
```{r}
...
- visualize a heatmap of the movie rating
```{r}
